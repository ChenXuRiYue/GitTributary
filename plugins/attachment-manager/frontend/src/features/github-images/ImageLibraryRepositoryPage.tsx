  ...
  return (
    <div className="mx-auto w-full max-w-5xl pb-4">
      <div className="border-border/50 flex min-h-12 items-center gap-2 border-b pb-3">
        ...
        <div>
          <h2 className="gt-title-panel">图库仓库</h2>
      ...
      <section className="border-border/50 border-b py-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="gt-title-section">选择 Git 远程</h3>
          <Badge variant="outline" className="h-5 px-1.5 gt-caption">{remotes.length}</Badge>
        ...
        {remotes.length === 0 ? (
          <div className="text-muted-foreground gt-body border-border/60 border border-dashed px-4 py-8 text-center">
              ...
              return (
                <label key={remoteEntryKey(entry)} className="hover:bg-accent/20 flex cursor-pointer items-center gap-3 px-2 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="gt-body-strong flex items-center gap-2">
                      <span>{entry.name}</span>
                      {entry.credential_mode !== "system" && <Badge variant="secondary" className="h-5 px-1.5 gt-caption">Git 凭证</Badge>}
                    </div>
                    <div className="text-muted-foreground gt-caption truncate" title={entry.url}>{entry.url}</div>
                    <div className="text-muted-foreground gt-caption truncate" title={entry.repo_path ?? undefined}>{entry.repo_path}</div>
                  </div>
                </label>
              );
            })}
      ...
      {addingRemote && (
        <section className="border-border/50 border-b py-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h3 className="gt-title-section">添加到 Git 远程</h3>
          </div>
          <div className="grid gap-3 sm:grid-cols-[180px_minmax(220px,1fr)_minmax(180px,1fr)_auto] sm:items-end">
            ...
            <div>
              <label className="gt-label text-muted-foreground mb-1.5 block">Access Token</label>
      ...
      {draft.remote && (
        <section className="py-4">
          <div className="flex flex-wrap items-center gap-3">
            <div className="min-w-0 flex-1">
              <div className="gt-body-strong truncate">{selectedEntry?.name ?? draft.remote.name}</div>
              <div className="text-muted-foreground gt-caption truncate">{draft.remote.url}</div>
            </div>
            {check && (
              <span className="text-primary gt-caption flex items-center gap-1">
...
function Field({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <div>
      <label className="gt-label text-muted-foreground mb-1.5 block">{label}</label>
