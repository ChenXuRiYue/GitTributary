...
export function InventorySidebar({
  ...
  return (
    <aside
      ...
      style={{ width }}
    >
      <div className="border-border/30 na-title-section flex h-10 shrink-0 items-center border-b px-3">
        附件类型
      </div>
        ...
        {filters.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "na-body flex h-8 w-full items-center gap-2 rounded-md px-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50",
                filter === item.id
            ...
            >
              ...
              <span className="na-caption tabular-nums">{counts[item.id]}</span>
            </button>
          );
        })}
      </nav>
      {report && (
        <dl className="border-border/30 text-muted-foreground na-caption mt-auto space-y-1 border-t px-3 py-2">
