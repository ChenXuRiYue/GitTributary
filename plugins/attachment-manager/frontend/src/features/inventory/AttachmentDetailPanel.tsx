    ...
    <>
      ...
      >
        <div className="border-border/50 flex h-10 shrink-0 items-center justify-between gap-2 border-b px-3">
          ...
          <h2 className="gt-title-panel min-w-0 truncate" title={item.name}>{item.name}</h2>
        ...
        <div className="gt-thin-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 [scrollbar-gutter:stable]">
          ...
          <dl className="mt-4 grid grid-cols-[72px_minmax(0,1fr)] gap-x-2 gap-y-2">
            <dt className="text-muted-foreground gt-label">{item.kind === "link" ? "链接" : "路径"}</dt>
            <dd className="gt-caption break-all">{item.url ?? item.path}</dd>
            ...
            {item.kind === "link" && (
              <>
                ...
                <dd className="gt-body break-all">{item.domain ?? "未知"}</dd>
              </>
            )}
            <dt className="text-muted-foreground gt-label">大小</dt>
            <dd className="gt-body">{item.kind === "link" ? "远程资源" : formatBytes(item.size)}</dd>
            <dt className="text-muted-foreground gt-label">修改时间</dt>
            <dd className="gt-body">{item.kind === "link" ? "未知" : formatDate(item.modifiedAt)}</dd>
          </dl>
          ...
          <div className="border-border mt-4 border-t pt-4">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="text-muted-foreground gt-caption">{item.references.length}</span>
            </div>
