  ...
  return (
    <section className="flex h-full min-h-0 w-full overflow-hidden bg-background">
      <aside className="flex w-[300px] shrink-0 flex-col border-r border-border/60 bg-sidebar/70">
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border/50 px-4 py-3">
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="na-title-panel truncate">任务流</div>
              <Badge variant="outline" className="h-5 shrink-0 px-1.5 na-caption">{groups.length}</Badge>
            </div>
            <p className="na-caption mt-1 truncate text-muted-foreground">源仓库、范围、目标仓库</p>
          </div>
        ...
        <div className="na-thin-scroll min-h-0 flex-1 overflow-y-auto py-1">
          {groups.length === 0 ? (
            <div className="mx-3 my-4 rounded-md border border-dashed bg-background px-4 py-7 text-center">
              <div className="na-body-strong">还没有发布任务</div>
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
        {!draft ? (
          <div className="flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            <div className="na-title-panel mt-3">新建一个发布任务</div>
            <p className="na-body mt-2 max-w-sm text-muted-foreground">
          ...
          <>
            <div className="flex shrink-0 items-center justify-between gap-4 border-b border-border/60 px-6 py-4">
              <div className="min-w-0">
                <div className="flex min-w-0 items-center gap-2">
                  <div className="na-title-panel truncate">{draft.name || "未命名任务"}</div>
                  {draftIsCurrent && (
                    <Badge variant="secondary" className="h-5 shrink-0 px-1.5 na-caption">当前上下文</Badge>
                  )}
                  {dirty ? (
                    <Badge variant="outline" className="h-5 shrink-0 px-1.5 na-caption text-amber-600">未保存</Badge>
                  ) : (
                    <Badge variant="outline" className="h-5 shrink-0 px-1.5 na-caption text-muted-foreground">已保存</Badge>
                  )}
                </div>
            ...
            <div className="na-thin-scroll min-h-0 flex-1 overflow-y-auto">
              <div className="grid min-h-full xl:grid-cols-[minmax(320px,0.92fr)_minmax(440px,1.08fr)]">
                <div className="min-w-0 space-y-6 border-b border-border/50 px-6 py-5 xl:border-b-0 xl:border-r">
                  <section className="space-y-3">
                    <div>
                      <div className="na-body-strong">任务名称</div>
                      <p className="na-caption text-muted-foreground">例如「个人博客」「团队文档」「产品手册」。</p>
                    </div>
                    <Input
                      value={draft.name}
                  ...
                  >
                    <div className="space-y-3">
                      <p className="na-caption text-muted-foreground">只能从 Git 已配置的工作仓库中选择。</p>
                          ...
                          {selectedSource && (
                            <div className="rounded-md border bg-background px-3 py-2">
                              <div className="flex min-w-0 items-center gap-2">
                                ...
                                <div className="na-body-strong truncate">{selectedSource.name}</div>
                                <Badge variant="outline" className="ml-auto h-5 px-1.5 na-caption">
                                  {selectedSource.remotes.length > 0 ? `${selectedSource.remotes.length} remote` : "当前仓库"}
                                </Badge>
                              </div>
                              <p className="na-caption mt-1 truncate text-muted-foreground">{selectedSource.path}</p>
                            </div>
                          )}
                        </>
                      ) : (
                        <div className="rounded-md border border-dashed bg-background px-4 py-5 text-center">
                          <div className="na-body-strong">暂无已配置工作仓库</div>
                  ...
                  <section className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="na-body-strong">环境变量</div>
                        <p className="na-caption text-muted-foreground">保存此任务需要复用的非敏感变量。</p>
                      </div>
                    ...
                    {draft.env.length === 0 ? (
                      <div className="rounded-md border border-dashed px-4 py-5 text-center">
                        <div className="na-body-strong">暂无环境变量</div>
                        <p className="na-caption mt-1 text-muted-foreground">可以先为空,后续发布链路再读取。</p>
                      </div>
                    ) : (
                  ...
                  <CollapsibleConfigSection
                    ...
                    status={(
                      <div className="flex shrink-0 items-center gap-1">
                        ...
                        <Badge variant="outline" className="h-5 px-1.5 na-caption">{publishCandidates.length} 个候选</Badge>
                      </div>
                    )}
                  >
                    <div className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_minmax(340px,0.86fr)]">
                      <div className="min-w-0 space-y-2">
                        <div className="flex items-center justify-between gap-2">
                          <p className="na-caption text-muted-foreground">优先选择独立 Pages 仓库。</p>
                        ...
                        {publishCandidates.length === 0 ? (
                          <div className="flex min-h-40 flex-col items-center justify-center rounded-md border border-dashed bg-background px-4 py-6 text-center">
                            ...
                            <div className="na-body-strong mt-2">暂无可用发布仓库</div>
                          ...
                          <div className="na-thin-scroll max-h-[360px] space-y-2 overflow-y-auto pr-1">
                        ...
                        {target && selectedCandidate ? (
                          <>
                            <div className={cn(
                              ...
                              <div className="min-w-0">
                                <div className="na-body-strong">{selectedCandidate.reason}</div>
                            ...
                            <div className="space-y-1.5">
                              <span className="na-caption text-muted-foreground">GitHub Pages 源</span>
                            ...
                            <div className="grid gap-3 sm:grid-cols-2">
                              <label className="space-y-1.5">
                                <span className="na-caption text-muted-foreground">目标分支</span>
                                <Input
                              ...
                              <label className="space-y-1.5">
                                <span className="na-caption text-muted-foreground">发布目录</span>
                                <Input
                            ...
                            <label className="block space-y-1.5">
                              <span className="na-caption text-muted-foreground">Pages URL</span>
                            ...
                            <label className="block space-y-1.5">
                              <span className="na-caption text-muted-foreground">提交信息</span>
                              <Input
                          ...
                          <div className="flex min-h-44 flex-col items-center justify-center rounded-md border border-dashed bg-background px-4 py-6 text-center">
                            ...
                            <div className="na-body-strong mt-2">选择一个发布仓库</div>
                            <p className="na-caption mt-1 max-w-sm text-muted-foreground">
