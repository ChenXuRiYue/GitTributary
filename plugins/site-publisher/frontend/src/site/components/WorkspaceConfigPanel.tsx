...
export function WorkspaceConfigPanel({
  ...
  const confirmDiscardIfDirty = () => {
    ...
    return window.confirm("当前任务详情还有未保存的改动，查看其他任务/新建/删除会丢弃这些改动。确定要继续吗？");
  };

  const handleViewGroup = (id: string) => {
    if (id === viewingGroup?.id) return;
  ...
  const handleSetCurrentGroup = () => {
    if (!draft || draft.id === activeGroupId || dirty) return;
    onSelectGroup(draft.id);
  };
