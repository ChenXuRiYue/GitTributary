...
NoteAura 是一个面向 **Git + Markdown 笔记仓库** 的桌面增强工具。它不替代你喜欢的编辑器，而是站在仓库旁边，把 Git 留痕、GitHub 备份、Pages 发布、自动化 Flow 和本地数据中心组织成一套更现代的笔记工作台。

如果你喜欢用 Typora、Vim、VS Code 或普通文本编辑器写笔记，又希望笔记能自然沉淀出历史、发布站点、智能分析、长期记忆和自我激励，NoteAura 就是为这类工作流准备的。
...
很多笔记软件会把编辑器、数据库、同步、发布和 AI 都包在同一个系统里。NoteAura 选择另一条路：让创作继续发生在你已经顺手的 Markdown 工作流里，把增强能力放到 Git 仓库这一层。
...
| 数据 | 管理 NoteAura 自身的配置、环境和跨设备同步数据。 | 命名空间浏览、KV 搜索/删除/compact、Profile/环境、远程同步配置。 |
...
当前发布的构建结果如下（[NoteAura 文档发布 Demo](https://chenxuriyue.github.io/na-site-demo/pages/README.html)）

<a href="https://raw.githubusercontent.com/ChenXuRiYue/image-cloud/main/global/image-20260705152741858.png"><img src="https://raw.githubusercontent.com/ChenXuRiYue/image-cloud/main/global/image-20260705152741858.png" alt="NoteAura published documentation demo" width="920"></a>

### Flow：把重复的笔记动作自动化

Flow 是 NoteAura 的自动化模块。它采用 GitHub Actions 子集风格的 YAML，把事件、节点和执行结果连接起来。首版已经可以管理 Flow 文件夹和 YAML 草稿，浏览事件/节点目录，并手动运行流程。后续自动同步、定时任务、文件监听和 Agent 驱动流程都会从这里生长。
...
数据模块管理 NoteAura 产生的配置与状态，包括本地 JSONL 数据、命名空间、Profile、环境、凭据状态和远程配置中心同步。它现在承担“配置中心”的角色，未来会继续承载记忆、个性化偏好、跨设备状态和 Agent 上下文。
...
NoteAura 的长期目标不是再造一个封闭笔记软件，而是把 Git 笔记仓库升级成可分析、可发布、可自动化、可陪伴的个人知识系统。
...
- Tailwind CSS 4
- Radix UI primitives
- 内部 Rust crates: `na-git`, `na-files`, `na-data`, `na-flow`（均不可发布）
