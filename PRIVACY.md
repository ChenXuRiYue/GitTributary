...
本文档说明 NoteAura 在本机和用户配置的远端服务中如何处理数据。适用范围为 NoteAura 0.1.x。
...
当你选择一个仓库或配置一项任务时，NoteAura 可能读取：
...
NoteAura 会在本机保存必要配置和状态。默认应用级数据目录为：
...
NoteAura 会尽量将敏感字段作为私有记录处理，不应把 token 写入普通公开配置或发布输出。

你仍然应该遵守以下安全习惯：

- 不要把 `~/.noteaura/` 整体提交到公开仓库。
...
当你主动使用 GitHub 备份、Pages 发布、远程仓库管理或数据中心同步时，NoteAura 会根据你的配置向目标仓库执行 clone、fetch、pull、commit、push 等操作。
...
卸载应用后，如需清理 NoteAura 的本地配置与状态，可删除：
