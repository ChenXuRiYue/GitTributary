...
//!
//! ```text
//! cargo test -p na-git --release --test performance_test -- --ignored --nocapture --test-threads=1
//! ```
//!
//! 每项预算都可通过环境变量覆盖。查找顺序为：
//! `NA_PERF_<PROFILE>_<OP>_P95_MS`、`NA_PERF_<OP>_P95_MS`、内置预算。
//! 例如：`NA_PERF_LARGE_STATUS_P95_MS=2500`。CI 可以统一设置
//! `NA_PERF_BUDGET_MULTIPLIER=1.25` 吸收共享 runner 抖动，但不应关闭断言。
...
fn budget(profile: &str, operation: &str, default_ms: u64) -> Duration {
    ...
    let operation_key = format!("NA_PERF_{}_P95_MS", operation.to_ascii_uppercase());
    let configured_ms = env_number(&profile_key)
        .or_else(|| env_number(&operation_key))
        .unwrap_or(default_ms as f64);
    let multiplier = env_number("NA_PERF_BUDGET_MULTIPLIER").unwrap_or(1.0);

    assert!(configured_ms > 0.0, "{profile_key} must be positive");
    assert!(
        multiplier > 0.0,
        "NA_PERF_BUDGET_MULTIPLIER must be positive"
    );
...
fn build_fixture(profile: FixtureProfile) -> Fixture {
    let temp_dir = tempfile::Builder::new()
        .prefix(&format!("na-git-perf-{}-", profile.name))
        .tempdir()
...
fn add_history(path: &Path, additional_commits: usize) {
    ...
    let signature = Signature::new("NoteAura Perf", "perf@example.invalid", &fixed_time())
