...
use gt_store::{Store, SyncConfig, SyncEngine};
use serde_json::json;
...
fn test_latest_with_ts_and_set_with_ts_preserve_timestamp() {
    ...
    // set_with_ts 保留指定 t
    store.set_with_ts("settings", "k", json!("v1"), 100).unwrap();
    ...
    store.set_with_ts("settings", "k", json!("v2"), 200).unwrap();
    let latest = store.latest_with_ts("settings");
    let k = latest.get("k").expect("k 应存在");
...
fn test_export_excludes_private_namespaces() {
    ...
    assert!(env_data.join("settings.jsonl").exists(), "public ns 应被导出");
    assert!(
...
fn test_export_then_import_lww() {
    ...
    store_a.set_with_ts("settings", "a", json!("A1"), 100).unwrap();
    store_a.set_with_ts("settings", "b", json!("B1"), 200).unwrap();
    ...
    engine_a.export_public_to_checkout(&store_a, &checkout_a).unwrap();
    ...
    engine_b.import_public_from_checkout(&mut store_b, &checkout_a).unwrap();
    ...
    store_b.set_with_ts("settings", "c", json!("C1"), 150).unwrap();
    store_b.set_with_ts("settings", "a", json!("A2"), 50).unwrap();

    let checkout_b = base_b.join("databases").join("b");
    fs::create_dir_all(&checkout_b).unwrap();
    engine_b.export_public_to_checkout(&store_b, &checkout_b).unwrap();

    // A 从 B 的 checkout import:
    //  - a:远端 t=50 < 本地 t=100 → 保留 A1
    //  - b:远端 t=200 >= 本地 t=200 → 保持 B1
    //  - c:本地无 → 引入 C1
    engine_a.import_public_from_checkout(&mut store_a, &checkout_b).unwrap();
    assert_eq!(store_a.get("settings", "a"), Some(json!("A1")), "远端更老的 a 不应覆盖本地");
    assert_eq!(store_a.get("settings", "b"), Some(json!("B1")));
    assert_eq!(store_a.get("settings", "c"), Some(json!("C1")), "新增的 c 应被引入");
}
...
fn test_import_skips_private_files_in_checkout() {
    ...
    engine.import_public_from_checkout(&mut store, &checkout).unwrap();
