...
use gt_flow::{record_from_value, record_to_value, workflow_key, FlowRecord};
...
impl<'a> FlowDefinitionRepository<'a> {
    ...
    pub fn get(&self, id: &str) -> Result<Option<FlowRecord>> {
        self.store
            .get(gt_flow::FLOW_NAMESPACE, &workflow_key(id))
...
impl<'a> FlowDefinitionRepositoryMut<'a> {
    ...
    pub fn delete(&mut self, id: &str) -> Result<()> {
        self.store
            .delete(gt_flow::FLOW_NAMESPACE, &workflow_key(id))?;
        Ok(())
    }
