...
pub const HOST_NAME: &str = "gt-plugin-host";
...
fn load_plugin(params: &serde_json::Value) -> Result<LoadedPlugin, String> {
    ...
    unsafe {
        ...
        let abi_version = *library
            .get::<AbiVersionFn>(b"gittributary_plugin_abi_version\0")
            .map_err(|error| error.to_string())?;
        if abi_version() != 1 {
            return Err("unsupported plugin ABI version".to_string());
        }
        let handle_request = *library
            .get::<HandleRequestFn>(b"gittributary_plugin_handle_request\0")
            .map_err(|error| error.to_string())?;
        let free_string = *library
            .get::<FreeStringFn>(b"gittributary_plugin_free_string\0")
            .map_err(|error| error.to_string())?;
        Ok(LoadedPlugin {
            _library: library,
            handle_request,
...
mod tests {
    ...
    use super::*;
    use gt_plugin_protocol::{event, method, Message, Request};
