...
"#;
    ...
    assert_eq!(nodes[1].uses, "noteaura/git/push@v1");
        ...
        steps: vec![
            FlowBuildStepRequest {
                ...
                inputs: BTreeMap::from([(
                    "path".to_string(),
                    "${{ gn.workspace.active_repo }}".to_string(),
                )]),
            },
            FlowBuildStepRequest {
...
"#;
    ...
    let registry = test_node_registry();
    let draft =
        na_flow::build_flow_draft_from_yaml(workflow.to_string(), &pool.catalog(), &registry)
            .unwrap();
