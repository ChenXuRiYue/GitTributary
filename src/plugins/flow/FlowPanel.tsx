...
function nodeTypeTone(type: string) {
  switch (type) {
    case "build":
...
function sortedNodeDefinitions(definitions: FlowNodeDefinition[]) {
  return definitions
    .slice()
    .sort((a, b) => a.node_type.localeCompare(b.node_type) || a.uses.localeCompare(b.uses));
}

function nodeSearchText(definition: FlowNodeDefinition) {
  return [
    definition.uses,
    definition.name,
...
function nodeMatchesQuery(definition: FlowNodeDefinition, query: string) {
  const normalized = query.trim().toLocaleLowerCase();
  if (!normalized) return true;
  return nodeSearchText(definition).includes(normalized);
}

function groupNodeDefinitionsByType(definitions: FlowNodeDefinition[]) {
...
export function FlowPanel() {
  ...
  const loadNodeDefinitions = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const list = await invoke<FlowNodeDefinition[]>("flow_node_catalog");
  ...
  const loadFlowNodes = useCallback(async (id: string | null) => {
    ...
    setLoadError(null);
    try {
      const list = await invoke<FlowNodeSpec[]>("flow_nodes", { id });
      setFlowNodes(list);
    } catch (error) {
      setFlowNodes([]);
      setLoadError(error instanceof Error ? error.message : String(error));
