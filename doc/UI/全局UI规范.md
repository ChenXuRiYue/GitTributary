# NoteAura 全局 UI 规范（macOS 风格）

> 本文是项目前端的统一视觉与组件规范。Core 模块、插件页面和新组件都应遵循本规范。
...
NoteAura 是桌面操作型 App，不采用网页营销页的大字号体系。字号管理参考三套行业通用方法：

- **Material Design Type Scale**：用 display/headline/title/body/label 这类角色管理文字，而不是按组件随手定字号。
- **Apple Human Interface Guidelines / SF 字体语义**：桌面工具应优先使用紧凑、清晰、稳定的文本层级；指标和标题突出，列表正文保持扫描效率。
- **Tailwind 默认字号刻度**：实现上仍落在 Tailwind 可理解的 11/12/13/15/18/22 等近似刻度，AI 后续生成页面时理解成本低。

本项目落地为 `src/index.css` 中的 `na-*` 角色类：

| 角色类 | 尺寸 | 用途 |
| --- | --- | --- |
| `na-title-app` | 18 / 24, 600 | App 主标题、Core Shell 顶部标题 |
| `na-title-panel` | 15 / 20, 600 | 当前面板或选中对象标题 |
| `na-title-section` | 13 / 18, 600 | 区块标题、表格/列表组标题 |
| `na-body` | 13 / 18, 400 | 默认正文、列表正文、说明主文本 |
| `na-body-strong` | 13 / 18, 500 | 表格关键值、路径节点、可点击行主文本 |
| `na-caption` | 11 / 14, 400 | 次要说明、路径、时间、辅助信息 |
| `na-label` | 11 / 14, 500 | 指标标签、字段名、短状态说明 |
| `na-tree` | 13 / 16, 400 | 文件树/对象树主行 |
| `na-tree-meta` | 11 / 14, 400 | 文件树副标题、扩展名、计数 |
| `na-metric` | 18 / 24, 600 | 仪表盘核心数字/状态值，保持克制，不做大屏看板感 |
| `na-metric-compact` | 15 / 20, 600 | 侧栏、窄卡片内的小指标 |
| `na-code` | 12 / 18, mono | YAML、路径、命令、事件名 |

强制规则：
- 新页面默认只使用这些角色类；`text-xs/text-sm/text-lg` 仅允许在 shadcn 基础组件或局部兼容旧代码时出现。
- 阅读模式中，正文与树保持 13px；指标必须使用 `na-metric` 或 `na-metric-compact`，不能被做成普通正文。
- 操作模式中，按钮仍遵循组件库默认 14px，但面板内容、运行记录、调试路径继续使用 `na-*`。
- 文件树、侧栏、表格行必须统一紧凑，默认行高 28～32px；不要为了“显得高级”把列表撑大。
- 主内容栏的留白以信息密度服务为准：工具型页面默认 `p-4`，只有文档阅读或长表单才允许 `p-6` 以上。
...
- **全局复用**:Git 二级栏、主侧边栏、工具栏弹窗等凡是「有限空间 + 项可能很多」的场景都用此模式
...
会随选择、状态、远程仓库名、计数、提示语而明显变化的内容,不应贴着主内容边缘展示。它需要放在有呼吸空间的标题区或局部面板内,让变化发生在页面内部,而不是在边缘制造跳动。

强制规则:
- 顶栏左缘只放稳定的上下文标题,不要把会频繁变化的长文本、仓库名、错误提示直接贴边。
- 顶栏仅做最上层资源坐标定位: 能力域、页面域、当前资源上下文。保存、发布、运行、提交、同步等业务操作和当前页面状态必须放进页面/面板内部。
- 顶栏需要统计时,采用两端反向模型: 左栏从左到右展示 `一级域 / 二级域 / 当前资源`; 右栏展示 `a:A b:B c:C / d:D e:E f:F` 的纯值表达。统计必须是域级聚合,不明文展示“一级域/二级域”结构标签,也不是页面动作或未保存状态。
- 如果变化内容必须靠边,右侧不能再堆固定项挤压它;固定操作应移入下一行、局部工具区或折叠菜单。
- 顶栏内的可变文本必须有稳定宽度、截断和图标占位,不能因状态切换改变相邻控件位置。
- 当前选中项名称较长或语义较弱时(如 `ui-state`、远程 URL),只作为上下文标题,不要和搜索、视图切换、压缩等工具混在同一行。
- 低频配置项(如数据中心远程仓库、Profile/环境选择)不放在高频浏览工具条的主路径上,应收进配置按钮或二级工具区。

设计意图:
- 用户浏览配置域时,视觉焦点应稳定落在域和内容上。
- 工具条应该安静,不因为选中对象、状态提示或仓库名变化而左右弹跳。
- 固定项不要成为右侧“墙”,否则左侧可变内容会被挤压得难看。
...
- **统一项模型**：侧边栏只渲染 `WorkbenchItem`；Core 模块与插件 view 在进入侧边栏前完成转换。
