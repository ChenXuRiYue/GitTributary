...
GitTributary 是桌面操作型 App，不采用网页营销页的大字号体系。字号管理参考三套行业通用方法：
...
本项目落地为 `src/index.css` 中的 `gt-*` 角色类：

| 角色类 | 尺寸 | 用途 |
| --- | --- | --- |
| `gt-title-app` | 18 / 24, 600 | App 主标题、Core Shell 顶部标题 |
| `gt-title-panel` | 15 / 20, 600 | 当前面板或选中对象标题 |
| `gt-title-section` | 13 / 18, 600 | 区块标题、表格/列表组标题 |
| `gt-body` | 13 / 18, 400 | 默认正文、列表正文、说明主文本 |
| `gt-body-strong` | 13 / 18, 500 | 表格关键值、路径节点、可点击行主文本 |
| `gt-caption` | 11 / 14, 400 | 次要说明、路径、时间、辅助信息 |
| `gt-label` | 11 / 14, 500 | 指标标签、字段名、短状态说明 |
| `gt-tree` | 13 / 16, 400 | 文件树/对象树主行 |
| `gt-tree-meta` | 11 / 14, 400 | 文件树副标题、扩展名、计数 |
| `gt-metric` | 18 / 24, 600 | 仪表盘核心数字/状态值，保持克制，不做大屏看板感 |
| `gt-metric-compact` | 15 / 20, 600 | 侧栏、窄卡片内的小指标 |
| `gt-code` | 12 / 18, mono | YAML、路径、命令、事件名 |

强制规则：
- 新页面默认只使用这些角色类；`text-xs/text-sm/text-lg` 仅允许在 shadcn 基础组件或局部兼容旧代码时出现。
- 阅读模式中，正文与树保持 13px；指标必须使用 `gt-metric` 或 `gt-metric-compact`，不能被做成普通正文。
- 操作模式中，按钮仍遵循组件库默认 14px，但面板内容、运行记录、调试路径继续使用 `gt-*`。
