...
> **实现状态**: 已实现为 `plugins/site-publisher` 插件，其 Rust backend 自包含 `gt-site`。本文保留早期方案背景，当前边界以 `doc/架构/项目模块现状.md` 为准。
...
```text
plugins/site-publisher/backend/crates/gt-site/
  src/
    lib.rs
    scan.rs
    config.rs
...
- 在发布插件 backend 中维护 `gt-site` crate。
- 实现仓库扫描和候选目录识别。
- 实现 Markdown 到 HTML。
...
```text
plugin: "dev.gittributary.site-publisher"
...
`gt-files`、Git、Store 和 shell Core 能力。
