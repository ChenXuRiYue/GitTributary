...
> **实现状态**: 已实现为 `plugins/site-publisher` 插件，其 Rust backend 自包含 `na-site`。本文保留早期方案背景，当前边界以 `doc/架构/项目模块现状.md` 为准。
...
这个功能不依赖云服务,也不要求用户安装 Node.js、Python 或额外命令行工具。插件前端负责配置和展示，插件 Rust backend 负责构建。

## 二、产品定位

静态 HTML 构建器不是博客托管系统。它是面向本地仓库的发布插件能力。
...
用户需要将仓库中的设计文档、操作说明和变更记录打包成静态交付物,交给不使用 NoteAura 的人浏览。
...
```text
plugins/site-publisher/frontend/src/site/
  SitePanel.tsx
  types.ts
  components/
    SiteConfigPanel.tsx
...
页面由 `plugins/site-publisher/manifest.json` 声明:
...
| `outputDir` | 输出目录 | `<repo>/.noteaura/site` |
| `siteTitle` | 站点标题 | 仓库名 |
...
插件 backend 内置 Rust crate:

```text
plugins/site-publisher/backend/crates/na-site/
  src/
    lib.rs
    scan.rs
    config.rs
...
```text
SitePanel.tsx
  → backend.invoke: site.scan / site.build / site.publish
  → Extension API: shell.openPath / shell.revealPath
```

sidecar 将 `backend.invoke` 分发给插件动态库；宿主仅为文件、Git、Store 和 shell 等
Core 能力做权限映射。
...
构建配置建议保存到 NoteAura 数据中心,不要第一版写入仓库。
...
```json
{
  ...
  "outputDir": "/Users/mi/project/demo/.noteaura/site",
...
- 在发布插件 backend 中维护 `na-site` crate。
- 实现仓库扫描和候选目录识别。
- 实现 Markdown 到 HTML。
...
| 插件体系 | 发布页面由 `site-publisher` 插件提供，安装后进入工作台 |
...
```text
plugin: "dev.noteaura.site-publisher"
view: "site-publisher.main"
entry: "frontend/dist/index.html"
```
...
plugins/site-publisher/backend/crates/na-site/           扫描、构建与发布能力
```
...
当前目标是用项目内插件打通静态站点的安装、运行和卸载闭环。

## 十九、待决问题

1. 输出目录默认放仓库内 `.noteaura/site` 还是用户数据目录下。
...
静态 HTML 构建器由发布插件 backend 自包含，并通过受控 Extension API 复用宿主的
`na-files`、Git、Store 和 shell Core 能力。
