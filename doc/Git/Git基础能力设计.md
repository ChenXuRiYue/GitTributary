...
> 本文档为 NoteAura **Rust 端 Git 模块**的能力发散与架构设想。
> 定位：作为项目的**基座服务层**，向上层所有插件（备份、复习、AI、数据库、部署…）提供统一的 Git 原语与加工数据。
> 设计哲学：**Git 不是某个插件的私有实现，而是平台级公共能力。**
>
> **实现状态**: P0/P1/P2 阶段(仓库生命周期、状态、提交、分支、远程、认证)已基本完成。P3(stash/安全恢复)、P4(数据挖掘)、P5(事件总线)尚未实现。事件广播能力已由 `na-flow` 的 EventPool + CloudEvents 部分承载。详见 `doc/架构/项目模块现状.md`。
...
NoteAura 的核心价值命题就是「用 Git 管理笔记」。几乎所有上层功能都围绕 Git 数据展开：
...
> 缓存可存为 `.noteaura/cache.json` 或 SQLite，放在仓库根目录（类似 Obsidian 的 `.obsidian/`）。
...
```
用户点击「提交并推送」
→ 备份插件调用 na-git.stage_all()
→ 备份插件调用 na-git.commit(message)
→ na-git 广播 CommitCreated
→ 备份插件调用 na-git.push("origin", "main")
→ na-git 广播 PushCompleted
→ 前端收到事件，显示成功 toast
```
...
```
定时器触发（每 30 分钟）
→ na-git.status() → 判断有无变更
→ 有变更 → na-git.stage_all() + na-git.commit(auto_message)
→ na-git.push(...)
→ 无变更 → skip
```
...
```
复习插件初始化
→ 调用 na-git.file_last_modified_map()
...
→ 通知 na-git.refresh_status()  (或直接 status_file)
→ na-git 广播 StatusChanged
→ 备份面板 UI 自动刷新变更列表
...
│   ├── event.rs          # 事件总线：GitEvent + 订阅/广播         [未实现，已由 na-flow EventPool 部分替代]
...
1. **唯一入口**：整个应用只有 `na-git` 持有 `git2::Repository`，其他模块禁止直接 `use git2`。
...
8. **不侵入笔记仓库**：na-git 产生的缓存/配置文件统一放 `.noteaura/` 目录（可全局 gitignore 或写入仓库 `.gitignore`）,不污染 git history。

---

## 七、与 Tauri / 前端的接口契约

na-git 本身是纯 Rust crate。Tauri 层做薄胶水:
...
4. **子模块（submodule）**：图片伴生库如果作为 submodule 引入,na-git 是否需要支持 submodule 操作？
...
> na-git 是 NoteAura 的心脏：唯一持有仓库句柄,暴露领域级接口,广播状态事件。
