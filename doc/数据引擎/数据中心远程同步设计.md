...
> 本文档讨论 NoteAura 数据中心的远程同步补齐方案。
> 目标:基于 GitHub 仓库完成 `~/.noteaura/` 的跨端配置同步,同时保持模块职责清晰。

> **实现阶段(2026-06-25)**:
> - **Phase 1(已实现,传输模型)**:Store 本体仍在 `~/.noteaura/data/`;远程配置数据库 clone 到 `~/.noteaura/databases/<repo-name>-<url-hash>/`(配置数据库本体,不是缓存)。sync 在 checkout 上执行 `pull → import(LWW) → export → commit → push`,通过 export/import 在 Store 与 checkout 的 `environments/<env>/data/` 之间搬运 public 数据。private 靠 export 过滤 + checkout `.gitignore` 双保险。非 ff 直接报错。环境切换 UI 仍走本地 profile,未接到 sync env(默认 `default`)。
> - **Phase 2(待实现,re-root 模型)**:Store 按可见性分流,public 命名空间 `data_dir` 直接指向 checkout 的 `environments/<env>/data/`;private 物理迁到 `~/.noteaura/local/data/`(永不进 checkout);`~/.noteaura/` 不再是 git 仓库;环境切换变 reroot。彻底消除"checkout clone 了没人用"与 credential 物理隔离两缺口。
...
3. `na-data::sync` 直接使用 `git2` 执行 init/pull/push,职责越过了数据中心边界。
...
| 模块 | 职责 |
| --- | --- |
| `na-git` | 管理 GitHub 远程仓库配置、认证、仓库标签,并提供 clone/fetch/pull/push 等 Git 操作接口 |
| `na-data` | 管理 `.noteaura` 配置数据、命名空间、环境配置、同步策略、数据合并,并编排配置同步 |
| `na-flow`/调度层 | 监听事件、定时触发同步任务、串行化任务执行 |
| 前端数据中心面板 | 选择配置数据库、查看同步状态、触发手动同步、展示冲突 |

一句话:数据中心负责"什么时候同步、同步哪个配置数据库、clone 到哪里、哪些 `.noteaura` 数据需要合并";Git 模块负责"给定远程、认证和本地路径后,如何安全执行 Git 操作"。
...
配置数据库是一个由数据中心选择和编排、由 Git 模块执行 Git 操作的 Git 仓库,用于承载 NoteAura 的公共配置数据:
...
```text
https://github.com/ChenXuRiYue/na-config.git
```
...
1. 先确定一个远程同步仓库。它是配置中心容器,负责承载 NoteAura data-center 的远端数据。
...
```json
{
  "active_database_id": "github:ChenXuRiYue/na-config",
...
- 数据中心当前是全局 `~/.noteaura/`,不是多租户 Store。
...
| 字段 | 示例 | 说明 |
| --- | --- | --- |
| `id` | `github:ChenXuRiYue/na-config` | 稳定 ID |
| `name` | `NoteAura Config` | 展示名 |
| `remote_url` | `https://github.com/ChenXuRiYue/na-config.git` | 远程 URL |
...
这里不要求 Git 模块为配置数据库提供 `local_path`。普通内容仓库可以有自己的工作区路径,但配置数据库的 checkout 位置由数据中心决定,例如 `~/.noteaura/databases/<database-id>/`。

数据中心选择配置数据库时,只从 Git 模块返回的候选列表中选择:

```rust
na_git.list_registered_repos(filter: tag = "config-db")
```
...
本设计遵守 `数据流与响应链路规范.md` 中的强制约束:data-center 数据的存储、拉取、同步、合并只由数据引擎(`na-data`)实现;Git 仓库操作原语和 Git 事件信号由 Git 模块(`na-git`)实现。

配置中心仓库还有一条额外强制约束:只能使用显式配置的配置中心专用 Access Token。配置中心承载 NoteAura 内部 data-center 数据,并会在数据引擎中执行 clone、拉取、提交、推送等同步动作,因此不能隐式依赖系统 Git 凭据、SSH Agent、普通仓库 SSH Key 或 Git 模块的全局 token。Token 只在本机 `private.credentials/data_center.config_repo.token` 保存,不进入 public 配置与远端仓库。
...
保存并绑定成功后,该配置中心仓库必须进入 Git 模块的远程仓库配置视图,作为 NoteAura 级远程配置可见、可管理:

- `source = noteaura_config`
...
建议放在 `~/.noteaura/sync/database.json`:

```json
{
  "active_database_id": "github:ChenXuRiYue/na-config",
  "active_environment_id": "staging",
  "database_checkout_path": "~/.noteaura/databases/na-config-<url-hash>",
  "enabled": true,
  "auto_sync": true,
  "interval_seconds": 300,
...
{
  "schema_version": 1,
  "database_id": "github:ChenXuRiYue/na-config",
  "created_by": "NoteAura",
...
Phase 1 采用传输模型:Store 本体仍在 `~/.noteaura/data/`,git 操作全部在 `databases/<id>/` checkout 上执行,通过 export/import 在 Store 与 checkout 的 `environments/<env>/data/` 之间搬运 public 数据。
...
sequenceDiagram
    autonumber
    participant UI as 前端 DataPanel
    participant Cmd as Tauri 命令层(lib.rs)
    participant Store as na-data(本地 Store)
...
| 访问控制 | checkout 是配置数据库本体;`~/.noteaura/` 由 na-data 独占读写 |
...
可能同时打开两个 NoteAura 进程,或用户手动运行 CLI。需要文件锁:

```
~/.noteaura/sync/locks/sync.lock
```
...
> 数据中心远程同步不是"在 store 里写一套 Git 客户端",而是让数据中心作为同步编排者选择配置数据库、决定本地 checkout 位置、调用 Git 模块接口,再用自己的 JSONL 合并语义把 `.noteaura` 公共配置稳定同步到 GitHub。
