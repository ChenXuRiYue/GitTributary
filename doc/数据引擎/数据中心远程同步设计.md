...
3. `gt-store::sync` 直接使用 `git2` 执行 init/pull/push,职责越过了数据中心边界。
...
| `gt-store` | 管理 `.git-tributary` 配置数据、命名空间、环境配置、同步策略、数据合并,并编排配置同步 |
...
本设计遵守 `数据流与响应链路规范.md` 中的强制约束:data-center 数据的存储、拉取、同步、合并只由数据引擎(`gt-store`)实现;Git 仓库操作原语和 Git 事件信号由 Git 模块(`gt-git`)实现。
...
sequenceDiagram
    ...
    participant Store as gt-store(本地 Store)
...
| 访问控制 | checkout 是配置数据库本体;`~/.git-tributary/` 由 gt-store 独占读写 |
